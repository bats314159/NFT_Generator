...
contract NFTCollection is ERC721URIStorage, Ownable, IERC2981 {
    ...
    uint256 private _mintPrice; // wei; 0 = free
    ...
    /**
     * @notice Public payable mint.  Caller receives one token; supply the
     *         per-token URI returned by the off-chain generator.
     * @param tokenURI_  IPFS metadata URI for this specific token
     *                   (e.g. "ipfs://Qm…/metadata.json").
     */
    function publicMint(string calldata tokenURI_) external payable {
        require(!_paused,                              "NFTCollection: minting is paused");
        require(msg.value >= _mintPrice,               "NFTCollection: insufficient payment");
        require(_nextTokenId <= MAX_SUPPLY,            "NFTCollection: would exceed max supply");
    ...
    /** @notice Update the public mint price (in wei).  Use 0 for free mints. */
    ...
    /** @notice Returns true when public minting is paused. */
